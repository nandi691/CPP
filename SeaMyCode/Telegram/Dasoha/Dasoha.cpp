...
void Dasoha::ProcessCommand()
{
	...
	{
			//Add handler
		SendMonthDetails(local_time->tm_mon+1,local_time->tm_year+1900);
	}
	else if(m_data == SLOTS_FOR_NEXT_MONTH)
	{
				//Add handler
		SendMonthDetails(local_time->tm_mon+1,local_time->tm_year+1900,true);
	}
...
void Dasoha::BookSlot()
{
	...
	std::string tmpStr = "";
	PrintSlotDetails(&kartha,tmpStr);
	...
	for(const Json::Value & value : jsonObj)
	{
			if (date == value["date"].asInt())
			{
				DasohaKartha kartha(value["name"].asString(),
									...
									value["phn"].asString());
				PrintSlotDetails(&kartha,retStr);
				return ;
			}
	}
	return;
...
void Dasoha::PrintSlotDetails(DasohaKartha *kartha,std::string &retStr)
{
	if(kartha == NULL)
	{
...
void Dasoha::ResetDasohaStruct()
{
	for(int index=0;index<31;index++)
	{
		m_kartha[index].m_name.clear();
		m_kartha[index].m_date = index+1;
		m_kartha[index].m_month = 0;
		m_kartha[index].m_year = 1900;
		m_kartha[index].m_address.clear();
		m_kartha[index].m_phn_num.clear();
	}
}
void Dasoha::SendMonthDetails(int month,int year,bool nextMonth){
	ResetDasohaStruct();
	...
	for(const Json::Value & value : jsonObj)
	{
		int index = value["date"].asInt() -1;
		if(index > 30)
			continue;
		m_kartha[index].m_date = value["date"].asInt();
		m_kartha[index].m_month = month;
		m_kartha[index].m_year = year;
		m_kartha[index].m_name = value["name"].asString();
		m_kartha[index].m_address = value["address"].asString();
		m_kartha[index].m_phn_num = value["phn"].asString();
	}
	PrintDasohaStruct();
	ResetDasohaStruct();
	return;
...
void Dasoha::PrintDasohaStruct()
{
	std::string retStr = "";
	for(int index=0;index<31;index++)
	{
		if(!m_kartha[index].m_name.empty())
		{
			PrintSlotDetails(&m_kartha[index],retStr);
			retStr += "\n\n";
		}
	}

	SendToBot(retStr+std::string("\n\nEnd of list"));
}
